...
#if !defined(UnityStereoTransformScreenSpaceTex)
#define UnityStereoTransformScreenSpaceTex(uv) (uv)
    ...
    half4 getAOColor(half ao, half2 uv)
    {
        ...
        half3 color = tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(uv)).rgb;
    ...
    half4 frag_hqbilateral(v_data_blur i) : SV_Target
    {
        ...
        half3 accum = tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(i.uv)).rgb * w0;
        accum += tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(i.uv1.zw)).rgb * w1;
        accum += tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(i.uv1.xy)).rgb * w2;
        accum += tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(i.uv2.zw)).rgb * w3;
        accum += tex2D(_MainTex, UnityStereoTransformScreenSpaceTex(i.uv2.xy)).rgb * w4;
