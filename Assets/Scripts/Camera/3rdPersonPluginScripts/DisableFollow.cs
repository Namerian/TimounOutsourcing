        ...
        void Update()
        {
            ...
            if (followDisabled)
            {
                if (activateMotionCheck)
                {
                    ...
                    if(cameraController.target.GetComponent<Rigidbody>().velocity.sqrMagnitude > motionThreshold )
                    {
                        Vector2 stickValue = XInput.instance.GetStickVector(PlayerID.One,GamePadAxis.StickRight);

                        if (stickValue.sqrMagnitude <= motionThreshold)
                        {

                            Invoke("ActivateFollow", timeToActivate);
                            //Debug.Log("Follow");
                            //follow.inBackToFrontRotation = false;
                            //follow.follow = true;
