...
using GameInput;
public class CameraControllerLogic : MonoBehaviour {
    ...
    public void SetCameraTarget(Transform t)
    {
        ...
        //charController.cam = this.transform;
    }
	
    void GetInput()
    {

        vOrbitInput = XInput.instance.GetStickVector(0, GamePadAxis.StickRight).y;
        hOrbitInput = XInput.instance.GetStickVector(0, GamePadAxis.StickRight).x;
    ...
    void OrbitTarget()
    {
        ...
        if (vOrbitInput <= -0.3f || vOrbitInput >= 0.3f)
        {
            orbit.xRotation += -vOrbitInput * orbit.vOrbitSmooth * Time.deltaTime;
        }
        if(hOrbitInput <= -0.3f || hOrbitInput >= 0.3f)
        {
            orbit.yRotation += -hOrbitInput * orbit.hOrbitSmooth * Time.deltaTime;
        }

        //Debug.Log("hOrbitInput" + hOrbitInput);
        

        orbit.xRotation = Mathf.Clamp(orbit.xRotation, orbit.minXRotation, orbit.maxXRotation);

        Debug.Log("xOrbitRotation" + orbit.xRotation);
    }
    ...
    void MoveToTarget()
    {
        ...
        //Debug.Log("yOrbitRotation" + orbit.yRotation);
        Debug.Log("xOrbitRotation" + orbit.xRotation);
    ...
    public class CollisionHandler
    {
        ...
        //Returns the distance where the camera needs to be from the target
