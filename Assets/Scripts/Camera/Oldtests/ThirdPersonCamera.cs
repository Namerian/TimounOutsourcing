...
public class PositionSettings
{
    ...
    public float distanceUp = 1.15f;

    public float AwayZoomMultipler = 3f;
    public float UpZoomMultiplier = 4f;
}
...
public class OrbitSettings
{
    ...
    public float maxXRotation = -75;
    public float minXRotation = -5;
    public float vOrbitSmooth = 150;
    public float hOrbitSmooth = 150;

    public float behindXRot = 0;
    public float behindYRot = 0;

    public Vector3 lastRotation = Vector3.zero;
    public float xRotationBehind = -20;
    public float yRotationBehind = -180;
}
...
public class ThirdPersonCameraLogic : MonoBehaviour,
    InputEvents.IActionHandler
{
    #region Variables (private)

    [SerializeField]
    private Transform behindXform;
    [SerializeField]
    ...
    private Vector3 cameraBehindPosition = Vector3.zero;
    private Vector3 lastLookDir;
    private float angleSmoothing =0f;
    #endregion
	...
	public enum CamStates
	{
        ...
        FreeBehind      // Only for movement, follows character from behind, but able to slightly move camera to view facing surrounding.
	}
    ...
    void Start ()
	{
		...
		characterOffset = followXform.position + new Vector3(0f, position.distanceUp, 0f);
		distanceUpFree = position.distanceUp;
		distanceAwayFree = position.distanceAway;
	...
	void LateUpdate()
	{
        if (ContextManager.instance.currentContext == InputContextList.Exploration)
        {
            viewFrustum = DebugDraw.CalculateViewFrustum(GetComponent<Camera>(), ref nearClipDimensions);

            // Pull values from controller/keyboard
            float rightX = XInput.instance.GetStickVector(PlayerID.One, GamePadAxis.StickRight).x;
            float rightY = XInput.instance.GetStickVector(PlayerID.One, GamePadAxis.StickRight).y;
            float leftX = XInput.instance.GetStickVector(PlayerID.One, GamePadAxis.StickLeft).x;
            float leftY = XInput.instance.GetStickVector(PlayerID.One, GamePadAxis.StickLeft).y;

            characterOffset = followXform.position + (position.distanceUp * followXform.up);
            ...
            //ExecuteCameraState(camState);
            /*if(follow.IsInLocomotion())
            {
                camState = CamStates.FreeBehind;
            ...
            //cameraBehindPosition = -followXform.forward * -position.distanceAway - followXform.up * position.distanceUp;


            #region Helper Behind Camera Object
            
            Vector3 behindLookDir;
            Vector3 behindCurLookDir;

            behindLookDir = Vector3.Lerp(followXform.right * (leftX < 0 ? 1f : -1f), followXform.forward * (leftY < 0 ? -1f : 1f), Mathf.Abs(Vector3.Dot(behindXform.forward, followXform.forward)));
            Debug.DrawRay(behindXform.position, behindLookDir, Color.white);


            // Calculate direction from camera to player, kill Y, and normalize to give a valid direction with unit magnitude
            behindCurLookDir = Vector3.Normalize(characterOffset - behindXform.position);
            behindCurLookDir.y = 0;
            Debug.DrawRay(behindXform.position, behindCurLookDir, Color.green);

            // Damping makes it so we don't update targetPosition while pivoting; camera shouldn't rotate around player
            behindCurLookDir = Vector3.SmoothDamp(behindCurLookDir, behindLookDir, ref velocityLookDir, lookDirDampTime);

            Vector3 behindTargetPosition = characterOffset + followXform.up * position.distanceUp - Vector3.Normalize(behindCurLookDir) * position.distanceAway;

            Debug.DrawLine(followXform.position, behindTargetPosition, Color.magenta);

            CompensateForWalls(characterOffset, ref behindTargetPosition);
            SmoothPosition(behindXform,behindXform.position, behindTargetPosition);

            #endregion
            switch (camState)
            {
                case CamStates.FreeBehind:

                    if (!follow.IsInLocomotion())
                    {
                        Debug.LogWarning("You shouldn't use FreeBehind camera mode when not moving");
                    }
                    ...
                    if (follow.IsInLocomotion())
                    {
                        ...
                        curLookDir = Vector3.Normalize(followXform.position - this.transform.position);
                        curLookDir.y = 0;

                        //Debug.Log("lookdir" + lookDir);
                        //Debug.Log("curlookdir" + curLookDir);
                        ...
                        Vector3 BehindPosition = Quaternion.Euler(orbit.xRotationBehind, orbit.yRotationBehind, 0) * -Vector3.forward * -distanceAwayFree;
                        BehindPosition += followXform.position;
                        
                        Debug.Log("BehindPos" + BehindPosition);
                        Debug.Log("RealPosition" + this.transform.position);

                        Vector3 templookDir = Vector3.Lerp(followXform.right * (leftX < 0 ? 1f : -1f), followXform.forward * (leftY < 0 ? -1f : 1f), Mathf.Abs(Vector3.Dot(Vector3.Normalize(followXform.position - BehindPosition), followXform.forward)));
                        Debug.DrawRay(BehindPosition, templookDir, Color.red);

                        Vector3 tempCurLookDir = Vector3.Normalize(followXform.position - BehindPosition);
                        tempCurLookDir.y = 0f;
                        Debug.DrawRay(BehindPosition, tempCurLookDir, Color.green);

                        tempCurLookDir = Vector3.SmoothDamp(tempCurLookDir, templookDir, ref velocityLookDir, camSmoothDampTime);
                        //Debug.Log("tempLookDir" + templookDir);
                        //Debug.Log("tempCurlookdir" + tempCurLookDir);

                        /*
                        if (rightX <= -rightStickThreshold || rightX >= rightStickThreshold)
                        {
                            orbit.behindYRot += -rightX * orbit.hOrbitSmooth * Time.fixedDeltaTime;
                            if (orbit.behindYRot <= 90)
                                orbit.behindYRot = 90;
                        ...
                        {*/
                            // Adjusting Orbit with current look direction angle
                            if (lastLookDir != Vector3.zero)
                            {
                            Debug.Log("asda");
                                Vector3 axisSign = Vector3.Cross(tempCurLookDir, lastLookDir);
                                angle = Vector3.Angle(lastLookDir, tempCurLookDir) * (axisSign.y >= 0 ? -1f : 1f);

                                orbit.yRotationBehind += angle;

                                if (orbit.yRotationBehind > 360f)
                                    orbit.yRotationBehind -= 360f;
                                if (orbit.yRotationBehind < -360f)
                                    orbit.yRotationBehind += 360f;

                            }
                            lastLookDir = tempCurLookDir;

                        //Smothing rotation to get back behind
                        /*if (orbit.behindYRot < 0.01 && orbit.behindYRot > -0.01)
                        {
                            orbit.behindYRot = 0f;
                            //lastLookDir = curLookDir;
                            //isSmoothing = false;
                            //Debug.Log("Stop");
                        }
                        else
                        {
                            orbit.behindYRot *= 0.98f;
                            /*if (!isSmoothing)
                            {
                                Debug.Log("smooooothStart");
                        ...
                        float angleToMoveTo = orbit.behindYRot + orbit.yRotationBehind;

                            orbit.yRotation = angleToMoveTo;
                    }

                    //targetPosition = Quaternion.Euler(orbit.xRotation, orbit.yRotation, 0) * -Vector3.forward * -distanceAwayFree;
                    //targetPosition += followXform.position;

                    //targetPosition = followXform.position + Quaternion.Euler(orbit.xRotation, orbit.yRotation, 0) * cameraBehindPosition;
                ...
                case CamStates.Behind:
                    ...
                    targetPosition = characterOffset + followXform.up * position.distanceUp - Vector3.Normalize(curLookDir) * position.distanceAway;
                    Debug.DrawLine(followXform.position, targetPosition, Color.magenta);
                ...
                case CamStates.Target:
                    ...
                    targetPosition = characterOffset + followXform.up * position.distanceUp - lookDir * position.distanceAway;
                ...
                case CamStates.Free:
                    ...
                    if (rightY <= -rightStickThreshold || rightY >= rightStickThreshold && (orbit.xRotation >= orbit.maxXRotation && orbit.xRotation <= orbit.minXRotation))
                    {
                        orbit.xRotation += -rightY * orbit.vOrbitSmooth * Time.fixedDeltaTime;
                    ...
                    if (rightX <= -rightStickThreshold || rightX >= rightStickThreshold)
                    {
                        orbit.yRotation += -rightX * orbit.hOrbitSmooth * Time.fixedDeltaTime;
                    }

                    

                    //Clamping doesn't seem to work properly
                    //xOrbitRotation = Mathf.Clamp(xOrbitRotation, -5f, -85f);

                    Vector3 tempDestination = followXform.position /*+ Vector3.up * 0f + Vector3.forward * 1f + transform.TransformDirection(Vector3.right * 0f)*/;

                    

                    targetPosition = Quaternion.Euler(orbit.xRotation, orbit.yRotation, 0) * -Vector3.forward * -distanceAwayFree;
                    targetPosition += tempDestination;
            ...
            // Clamping Value
            if (orbit.yRotation > 360f)
                orbit.yRotation -= 360f;
            if (orbit.yRotation < -360f)
                orbit.yRotation += 360f;

            CompensateForWalls(characterOffset, ref targetPosition);
            SmoothPosition(cameraXform,cameraXform.position, targetPosition);

            transform.LookAt(lookAt);

            // Make sure to cache the unscaled mouse wheel value if using mouse/keyboard instead of controller
            //rightStickPrevFrame = new Vector2(rightX, rightY);//mouseWheel != 0 ? Scaled : rightY);
        }
	}
    ...
    private void SmoothPosition(Transform xForm, Vector3 fromPos, Vector3 toPos)
	{		
		// Making a smooth transition between camera's current position and the position it wants to be in
		xForm.position = Vector3.SmoothDamp(fromPos, toPos, ref velocityCamSmooth, camSmoothDampTime);
	}
    ...
    {
        /*
        if (action == InputActionList.FirstPersonView && state == InputState.Pressed && camState == CamStates.Free && !follow.IsInLocomotion() && !follow.IsInJump())
        {
            xAxisRot = 0;
            lookWeight = 0f;
        ...
        /*
        if (action == InputActionList.BehindCamera && state == InputState.Pressed && camState == CamStates.Free)
        {
        ...
        if (action == InputActionList.BehindCamera && state == InputState.Released && camState == CamStates.Target)
        {
            switchToFreeCam();
        }

        if (action == InputActionList.FirstPersonView && state == InputState.Released && camState == CamStates.FirstPerson)
        {
            switchToFreeCam();
        }
        */
